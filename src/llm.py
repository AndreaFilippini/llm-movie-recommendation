...
def get_llm_response(user_input, user_id):
    ...
    movies = fetch_movie_data(tmdb_token, num_movies=total_movies)
...
if __name__ == "__main__":
    ...
    if not hugging_token:
        print("ERRORE: HUGGING FACE token not found\n")
        sys.exit()

    if not tmdb_token:
        print("ERRORE: TMDB token not found\n")
        sys.exit()
    
    if not openai_token:
        print("ERRORE: OPENAI token not found\n")
        sys.exit()
